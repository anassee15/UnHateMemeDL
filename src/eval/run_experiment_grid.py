...
def main() -> None:
    ...
    for eval_type, exp_cfg in experiments.items():
        for model_cfg in exp_cfg["models"]:
            ...
            # Phase A: VLM (detection + prompts) — skipped if --mitigation_only
            if not args.mitigation_only:
                ...
                # With --mitigation_only: skip detection, go directly to prompt gen + diffusion
                print(f"\n[Phase A (mitigation only)] VLM: prompt generation ({n_reps} reps)", file=sys.stderr)
