...
The hateful meme detection step (`src/eval/run_detection_eval.py`) supports 7 prompting strategies via `--pipeline`:

| Pipeline | Description | VLM calls |
|---|---|---|
| `zeroshot` **(default)** | Hate definition + classification criteria only. No examples, no affect routing. Compatible with the baseline behavior in `main`. | 1 |
...
Quantitative comparison across all pipelines: see [`report/pipeline_comparison.md`](report/pipeline_comparison.md).
