...
def parse_affect_response(response):
    if not isinstance(response, str):
        raise TypeError("response must be a string")

    raw = response.strip()
    if not raw:
        return _default_affect("Empty affect classification response")
    ...
    try:
        ...
        return _default_affect("Invalid JSON in affect classification response")
    ...
    for key, valid_values in allowed.items():
        value = str(parsed.get(key, defaults[key])).strip().lower()
        if value not in valid_values:
            value = defaults[key]
        affect[key] = value

    affect["rationale"] = str(parsed.get("rationale", "")).strip()

    return affect


def _default_affect(reason: str) -> dict:
    return {
        "overall_sentiment": "neutral",
        "humor": "not_funny",
        "sarcasm": "not_sarcastic",
...
def _fallback(prompt: str, reason: str) -> dict:
    ...
    return {
        ...
        "strategy": "Fallback: no mitigation applied",
        "flux_prompt": prompt,
        "expected_change": "Image unchanged due to parse error",
        "_parse_error": reason,
    }